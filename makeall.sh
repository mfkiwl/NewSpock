...
if [ $is_sift -eq 1 ];then
    ...
    path_gsl=/opt/local #./gsl_installation # when distributing: ./gsl_installation
else
    path_spice=./cspice # /Users/cbv/cspice   #$3 don't include the last '/' in the path
    path_gsl=/opt/local # /usr/local    #$4 you can put ./ for current directory but don't put ../ for one level up. in this case, put absolute path. also, don't include the last '/' in the path
fi

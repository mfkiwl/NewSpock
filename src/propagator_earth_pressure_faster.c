...
// in the main output file, the average is reported as 9999.999999
// unless eaxctly one full orbit has been travelled, in which case the
// average fo the parameter over this orbit is reported and the string
// "ORB" is indluced at the end of the output file
// --------------------------------------------------------------------
...
int orbit_ave(SPACECRAFT_T *SC,
  ...
  int jump360 = 0;
  // this condition occurs when th sc crosses the AN
  if ( fabs(SC->OE.an_to_sc - previous_an_to_sc) > 350/RAD2DEG) jump360 = 1;
  
  // the sc has traveled a multiple of one orbit around the Earth
  // since the start of the simu
  if ( ( ( (SC->OE.an_to_sc >= SC->OE.initial_an_to_sc) &&
	   ( previous_an_to_sc < SC->OE.initial_an_to_sc) ) ||
	 ((SC->OE.an_to_sc <= SC->OE.initial_an_to_sc) &&
	  ( previous_an_to_sc > SC->OE.initial_an_to_sc))) && (jump360 == 0) ) {
...
// --------------------------------------------------------------------
// convert ECI xyz to lon/lat/alt. x,y,z in km. time in seconds past J2000.
// --------------------------------------------------------------------
...
int eci2lla(double pos[3], double et,  double geodetic[3]  ){ 
  ...
  // // convert time string to year month day hour minute seconds (all
  // int, except seconds which is double)

  int year_current, month_current, day_current, hour_current, minute_current;
  double second_current;
  char year_current_str[10];	
  strcpy(year_current_str, "");
  strncat(year_current_str, &cu_time[0], 4);
  year_current = atoi(year_current_str);
  char month_current_str[10];	
  strcpy(month_current_str, "");
  strncat(month_current_str, &cu_time[5], 2);
  month_current = atoi(month_current_str);
  char day_current_str[10];	
  strcpy(day_current_str, "");
  strncat(day_current_str, &cu_time[8], 2);
  day_current = atoi(day_current_str);
  char hour_current_str[10];	
  strcpy(hour_current_str, "");
  strncat(hour_current_str, &cu_time[11], 2);
  hour_current = atoi(hour_current_str);
  char minute_current_str[10];	
  strcpy(minute_current_str, "");
  strncat(minute_current_str, &cu_time[14], 2);
  minute_current = atoi(minute_current_str);
  char second_current_str[10];	
  int second_current_no_microsec;
  strcpy(second_current_str, "");
  strncat(second_current_str, &cu_time[17], 2);
  second_current_no_microsec = atoi(second_current_str);
  char microsecond_current_str[10];	
  int microsecond;
  strcpy(microsecond_current_str, "");
  strncat(microsecond_current_str, &cu_time[20], 3);
  microsecond = atoi(microsecond_current_str);
  second_current = second_current_no_microsec + microsecond/1000.;

  double time;
  // // Convert year month day hhour min sec to Julian day (in ut1
  // days from 4713 bc)

  jday (year_current, month_current, day_current,
	hour_current, minute_current, second_current, &time );
...
*
*  author        : Charles Bussy-Virat, largely inspired from
*                   SGP4 by david vallado 
*
...
*
*  author        : Charles Bussy-Virat, largely inspired from SGP4 by 
*                  david vallado
*
*  inputs          description                    range / units
*    year        - year                           1900 .. 2100
...
int jday(int year, int mon, int day,
	 int hr, int minute, double sec, double *jd ) {
  *jd = 367.0 * year -
...
//
//  Purpose: Computes the coverage of a satellite above all ground
//  stations and calculate a few parameters: elevation, azimuth, range
//  in different two coordinate systems (ECEF, ENU) and in two
//  different frames of reference (satellite and ground station)
//
//  Assumptions: if the sc is a GPS that was initialized in the second
//  line of section #SPACECRAFT of the main input file, its attitude
//  is assumed to be nadir pointing
//
//  References:     /
//
//  Change Log:
...
SPACECRAFT_T *SC = spacecraft (position and time). out: elevation,
  azimuth, range

GROUND_STATION_T *GROUND_STATION = list of ground stations

PARAMS_T *PARAMS = parameters for the propagation (Earth flattening
  and radius here)

int index_in_attitude_interpolated = index of the current time step
  (take into account RK4)

INTEGRATOR_T *INTEGRATOR = paramters for the propagation (attitude of
  the sc here)

double et_initial_epoch = time of the inital epoch of the
  constellation

double et_sc_initial

double sc_ecef_previous_time_step[3] = ECEF position of the spacecraft
  at the previous time step of the propagation (used to linear
  interpolate the position between the previous position and the current
  position)

double sc_eci_previous_time_step[3] = ECI position of the spacecraft
  at the previous time step of the propagation (used to linear
  interpolate the position between the previous position and the current
  position)

double sc_v_eci_previous_time_step[3] = ECI velocity of the spacecraft
  at the previous time step of the propagation (used to linear
  interpolate the position between the previous position and the current
  position)

double time_step_interpolation = time step of the linear interpolation
  of the sc position (in seconds)
 */

int coverage_ground_station(SPACECRAFT_T *SC,
			    ...
			    double et_sc_initial,
			    double sc_ecef_previous_time_step[3],
			    double sc_eci_previous_time_step[3],
			    double sc_v_eci_previous_time_step[3],
			    double time_step_interpolation) {
  ...
  double ground_station_to_sc_eci[3];
  double ground_station_to_sc_lvlh[3];
  double ground_station_to_sc_body[3];
  double v_angle[3];
  ...
  double elevation_wtr_to_ground_station_in_sc_refce;
  double azimuth_wtr_to_ground_station_in_sc_refce;
  double range_wtr_to_ground_station;
  double elevation_wtr_to_ground_station_in_ground_station_refce;
  double azimuth_wtr_to_ground_station_in_ground_station_refce;
  int istep;
  ...
  for (iground = 0; iground < GROUND_STATION->nb_ground_stations; iground++){
    ...
    // Write the header at the first time step of the propagation (the
    // second condition is in case the initialization was made with
    // TLEs, the fisrt time to write is when the SC->et is eqaual to
    // initial_epohc (we dont want to write at the first time step
    // right after the TLE initial epoch_

    if ( (index_in_attitude_interpolated ==
	  INTEGRATOR->index_in_attitude_interpolated_first + 2) ||
	 (fabs(et_initial_epoch - SC->et) <= INTEGRATOR->dt/2.)){ 
      ...
      fprintf(SC->fp_coverage_ground_station[iground],
	      "// Coverage of Spacecraft %s for the ground station %s.\n",
	      SC->name_sat,GROUND_STATION->name_ground_station[iground]);
      fprintf(SC->fp_coverage_ground_station[iground],
	      "// \n");
      fprintf(SC->fp_coverage_ground_station[iground],
	      "// Version control is under Joel Getchius' Mac and Charles Bussy-Virat University of Michigan's Mac \n");
      fprintf(SC->fp_coverage_ground_station[iground],
	      "// \n");
      fprintf(SC->fp_coverage_ground_station[iground],
	      "// -------------------------------------------------------- \n");
      fprintf(SC->fp_coverage_ground_station[iground],
	      "// Ground station %s ECEF: %f %f %f\n",
	      GROUND_STATION->name_ground_station[iground],
	      GROUND_STATION->ecef_ground_station[iground][0],
	      GROUND_STATION->ecef_ground_station[iground][1],
	      GROUND_STATION->ecef_ground_station[iground][2]);
      fprintf(SC->fp_coverage_ground_station[iground],
	      "// Ground station %s longitude(degree) latitude(degree) altitude(m): %f %f %f\n",
	      GROUND_STATION->name_ground_station[iground],
	      GROUND_STATION->longitude_ground_station[iground]*RAD2DEG,
	      GROUND_STATION->latitude_ground_station[iground]*RAD2DEG,
	      GROUND_STATION->altitude_ground_station[iground]);
      fprintf(SC->fp_coverage_ground_station[iground],
	      "TIME ECEF_X_SC ECEF_Y_SC ECEF_Z_SC ELEV_REFCE_SC AZI_REFCE_SC ELEV_REFCE_GROUND_STATION AZI_REFCE_GROUND_STATION RANGE_SC_TO_GROUND_STATION\n");
      fprintf(SC->fp_coverage_ground_station[iground],
	      "#START \n");
    }
    ...
    if ((index_in_attitude_interpolated ==
	 INTEGRATOR->index_in_attitude_interpolated_first + 2)) {
      step_end_interpolation = (int)(nearbyint(INTEGRATOR->dt /
					       time_step_interpolation + 1));
      et_interpolated = et_previous_time_step - 1;
    } else{
      step_end_interpolation = (int)(nearbyint(INTEGRATOR->dt /
					       time_step_interpolation));

...
//  Assumptions:    Spice files have to be in directory_to_cspice/data/
//                   (where directory_to_cspice is indicated in the
//                     Makefile at SPICE_DIR)
//  References      Various
//
//  Change Log:
...
// C. Bussy-Virat| 07/22/2015    | Add orbital elements in the
//                               | input file and reorganize it
// C. Bussy-Virat| 07/23/2015    | Replace altitude by apogee of
//                                 altitude in the input file
// C. Bussy-Virat| 07/29/2015    | Added number of surfaces and normal
//                                 to the surface
// C. Bussy-Virat| 07/30/2015    | Read from different input files
//                                 (basic.d, orbit.d, spacecraft.d,
//                                  attitude.d, force.d)
// C. Bussy-Virat| 07/31/2015    | For each surface of the sc, read the
//                                 area, Cd, and normal vector
...
int load_options( OPTIONS_T *OPTIONS,
		  ...
		  int nProcs,
		  int iDebugLevel,
		  char filename_input_raw[256]) {

   char text_spice[256];

   // set this variable to 1 to prevent from downloading F10.7 and Ap
   // files from SWPC or Omniweb
   int do_not_download_file_swpc_or_wget = 0;
   ...
   // do not really care about this line (as long as the spacecrafts
   // are not flying after 2060)
   SpiceInt frstyr = 1961; 
   ...
   /* Set up the geophysical quantities.  At last check these were the
      values used by Space Command and SGP4 */
   ...
   int ooo;
   int quarter_initial_swpc, quarter_final_swpc;
   int quarter_current, quarter_initial_swpc_ap, quarter_initial_epoch;
   ...
   char initial_epoch_wget_minus40_5days_temp[12];
   char final_epoch_wget_plus40_5days_temp[12];
   ...
   int more_ensembles_output;
   int inc_ensembles_output = 0, inc_ensembles_output_final = 0;
   ...
   // filename_input_raw is the name of the input file as indicated by
   // the argument of the SpOCK call
   char filename_input_no_path[1000]; 
   // filename_input_no_path is filename_input_raw without the path to the file 
   strcpy(filename_input_no_path, "");    
   if (strrchr(&filename_input_raw[0], '/')== NULL){
     // in the name indicated at 1st line of section #OUTPUT
     strcpy(filename_input_no_path, filename_input_raw);
   } else {
     // '/' character in the name indicated at 1st line of section #OUTPUT
     strncat(filename_input_no_path,
	     strrchr(&filename_input_raw[0], '/')+1,
	     (int) (strrchr(&filename_input_raw[0], '/') +
		    strlen(filename_input_raw) ) -1);
   }

   fp = fopen(filename_input_raw, "r");
   ...
   // second file, and other files are). You need to read the leap second
   // file before reading section TIME. But then you need to read the
   // section TIME before loading the other SPICE kernels
   /* SPICE */
   if (iDebugLevel >= 2)
   ...
   // ASSUMPTION: the path of spice specified in the main input file
   // in the sectioN #SPICE must be the same as the path of SPICE
   // installation in the Makefile, with /data at the end. So if in the
   // Makefile the SPICE directory is hello/hi/ then the path of spice
   ...
   if ( (strlen(text_spice) == 0 ) || ( text_spice[0] == '#' ) ){
     // if the user left the line (below the path to SPICE) empty of if
     // it's the start of another section ('#' character) then it means
     // the user wants to use the default earth_binary_pck file
     strcpy(text_spice, "default");
   }

   // // Now load the leap second file. ASSUMPTION: it has to be in
   // directory_to_cspice/data/ (where directory_to_cspice is indicated
   // in the Makefile at SPICE_DIR)
   ...
   if ( strcmp(text, "now") == 0){
	 ...
	 for (ccc = 1; ccc < nProcs; ccc++)
       ...
       if (nProcs > 1)
	 MPI_Recv(&et_initial, 1, MPI_DOUBLE, 0, 0,
		  MPI_COMM_WORLD,  MPI_STATUS_IGNORE);
     }

     et2utc_c(et_initial,
	      "ISOC", 6, 255, OPTIONS->initial_epoch);
     // don't worry about it
     // Date of final epoch (= now + number of hours chosen in the
     // input file input.d)
     rewind(fp);
     found_eoh=0;
     while ( found_eoh == 0 && !feof(fp)) {
   ...
   // Re-evaluate OPTIONS->final epoch and et_end so that final epoch
   // is a multiple of dt + inital epoch (if originally it is not,
   // then take the closest multiple just before the orginal value of
   // final epoch (ex: if initial epoch is at 12:00:00, dt = 20s, and
   // final epoch as written in main input file is 12:00:50 then
   ...
   if ( fabs(fmod( et_end - et_start, OPTIONS->dt ) ) > 1e-6 ){
